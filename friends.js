...
// friend 
fetch(`http://127.0.0.1:8000/accounts/receive/accept/?account_id=${accountId}`)
.then(res => res.json())
.then(data_1 =>{
  // console.log(data_1)
  fetch(`http://127.0.0.1:8000/accounts/send/accept/?account_id=${accountId}`)
  .then(res => res.json())
  .then(data_2 =>{
    // console.log(data_1)
    let friends = []
    ...
    for (let i = 0; i < len; i++) {
      ...
      {
        fetch(`http://127.0.0.1:8000/accounts/profile/${friends[i].sender_account}/`)
        .then((res) => res.json())
        .then((account) => {
      ...
      {
        fetch(`http://127.0.0.1:8000/accounts/profile/${friends[i].receiver_account}/`)
        .then((res) => res.json())
        .then((account) => {
